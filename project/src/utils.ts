 ...
 */
export const getDescriptionPosition = function (point: { x: number; y: number }, texts: string[], astrology: {
 ...
 * @return {boolean}
 */
export const isCollision = (circle1: { x: number; y: number; r: number }, circle2: {
    x: number;
    y: number;
    r: number
}): boolean => {
 ...
 */
export const assemble = (locatedPoints: LocatedPoint[], point: LocatedPoint, universe: {
    cx: number;
    cy: number;
    r: number
 ...
 */
export const getRulerPositions = (centerX: number, centerY: number, startRadius: number, endRadius: number, startAngle: number, astrology: {
    SHIFT_IN_DEGREES: number
}): InitialEndPosition[] => {
    const result = []
